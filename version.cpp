...
#define LSX_HEADER         ""
#define LSX_REQUEST_START  ""
// Note: LSX_RESPONSE_START and LSX_ENTITLEMENT would be defined here in the
// original binary; they are referenced later in InterceptLSXResponse but
// were elided from this reconstruction.
...
void LoadMainConfig()
{
    ...
    // Build path: %APPDATA%EA DLC Unlocker v2config.ini
...
void LoadGameConfig(const std::string& gameTitle)
{
    // Build path to game config: %APPDATA%EA DLC Unlocker v2configsg_The-Sims-4.ini
...
// Hooked implementation of QVector::data()
...
void InterceptLSXResponse(std::string& response)
{
    // NOTE: LSX_RESPONSE_START and LSX_ENTITLEMENT are referenced here but not
    // defined in this reconstruction snippet; they would correspond to the
    // literal tag strings in the original binary.

    // Find <Entitlements> section
#ifdef LSX_RESPONSE_START
    ...
    if (dwStatusCode == HTTP_STATUS_OK)
    {
        ...
        if (hFile != INVALID_HANDLE_VALUE)
        {
            ...
            WriteFile(
                hFile,
                responseData.c_str(),
                static_cast<DWORD>(responseData.length()),
                &dwBytesWritten,
                nullptr
            );
            CloseHandle(hFile);
